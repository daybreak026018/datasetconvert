...
1. 打开 `环境检测`，确认 PyTorch、CUDA、Ultralytics 是否可用。
2. 打开 `数据准备`，选择数据集目录并进行 YOLO 体检。
3. 对检测/分割数据集生成或修复 `data.yaml`。
4. 如需要，使用 `格式转换`、`数据划分`、`分析报告`、`可视化预览` 等旧数据准备功能。
5. 打开 `模型训练`，选择任务类型、模型、数据和训练参数。
6. 训练完成后进入 `结果管理`，找到 `best.pt` 或 `last.pt`。
7. 打开 `模型检测`，选择权重和输入源执行推理。
...
>>>>>>> 4166d792675e1dc775054cf7f0bc072cfc0f77ab
dataset/
  data.yaml
  images/
    train/
